...
class X_Api_V1_Strategy implements X_Api_Version_Strategy
{
	...
	{
		$response = $this->request->post("{$this->root_api}oauth/access_token", array(
			'body' => array(
	...
	public function auth_link(string $callback_url):string
	{
		...
		$headerAuth = $this->authorization_header($endpoint, array(
	...
	public function post(array $data, Network_Account $account): string
	{
		...
		$auth_headers = $this->authorization_header($endpoint, [
	...
	private function format_response(array|string $response, string $sep='oauth_'): mixed
	{
		...
		return array_reduce($token_arr, function (array $acc, $item) use($sep){
			if(!$item){
				return $acc;
			}
	 ...
	 * @throws Exception
	 */
	private function authorization_header(string $endpoint, array $params = [] ): string
	{
